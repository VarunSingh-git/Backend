...
const registerUser = asynchandler(async (req, res, next) => {
  ...
  if (
    (fullname.length < 2 && fullname.length == 0 && !email.includes("@")) ||
    email.startsWith("@") ||
    email.endsWith("@") ||
    email.split("@").length !== 2 ||
    email.split("@")[1].indexOf(".") === -1 ||
    email.split("@")[1].startsWith(".") ||
    (email.split(".").pop().length < 2 && password.length < 8) ||
    password.length == 0
  ) {
  ...
  if (existedUser)
    new apiError(409, "User with email and username already exist");
  ...
  const confirmCreatedUser = await User.findById(user._id)?.select(
    // yaha pe agr User.findById(user._id) iss id ke base pe user return ho toh using .select for remove that field password & refreshToken
...
const changeCurrentPassword = asynchandler(async (req, res) => {
  ...
  if (!(confirmPassword === newPassword))
    throw new apiError(401, "Confirm password does not match the new password");
  ...
  try {
    ...
    throw new apiError(
      400,
      error.message || "Error occure during save info in DB"
    );
  }
  const options = {
    httpOnly: true,
...
const changeCurrentInfo = asynchandler(async (req, res) => {
  ...
  if (
    [newFullName, newEmail, newUsername].some((field) => field?.trim() === "")
  ) {
    throw new apiError(400, "All Fields are required");
  }
  ...
  const userForValidation = await User.findById(req.user?._id).select(
    "-password -refreshToken"
  );
  ...
  if (
    ...
    throw new apiError(
      401,
      "Please change the data either you can go step back"
    );
...
const deleteCoverImg = asynchandler(async (req, res) => {
  const user = await User.findById(req.user?._id);
  if (!user || !user.coverImg)
    throw new apiError(401, "User or cover image not found");

  const url = user?.coverImg;
  const publicId = getPublicId(url);

  const deleteCoverImgFromCloudinary = deleteFromCloudinary(publicId);
  if (!deleteCoverImgFromCloudinary)
    throw new apiError(401, "Task failed due to server error... Try again");
...
const getUserChannelProfile = asynchandler(async (req, res) => {
  ...
  return res
    .status(200)
    .json(new apiResponse(200, channel[0], "User Chanel fetched"));
});
